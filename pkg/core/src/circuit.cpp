...
// **************************************************************************
bool Circuit::buildCircuit(Netlist* const pNetlist, const int& numFrame, const TIME_FRAME_CONNECT_TYPE& timeFrameConnectType) {
...
// Date       [ Ver. 1.0 started 2013/08/11 last modified 2023/01/05 ]
// **************************************************************************
void Circuit::createCircuitPmt(const int& gateID, const Cell* const cell, const Pmt* const pmt) {
    // Determine gate type.
    ...
    for (; it != ps.end(); ++it) {
        if ((*it)->top_ != cell->libc_ && (*it)->top_ != pmt) {
            ...
            PortSet temp = cell->top_->getNetPorts(nid);
        }
    }
...
// Date       [ Ver. 1.0 started 2013/08/11 last modified 2023/01/05 ]
// **************************************************************************
void Circuit::determineGateType(const int& gateID, const Cell* const cell, const Pmt* const pmt) {
    switch (pmt->type_) {
        case Pmt::BUF:
...
void Circuit::connectMultipleTimeFrame() {
    for (int i = 1; i < numFrame_; ++i) {
        ...
        if (timeFrameConnectType_ == CAPTURE) {
            for (int j = 0; j < numPPI_; ++j) {  // Change numFO_ and fanoutVector_ for PPOs.
        ...
        for (int j = 0; j < numGate_; ++j) {  // Read gate informations.
            int gateID = offset + j;
            ...
            if (circuitGates_[gateID].gateType_ != Gate::PPI && circuitGates_[gateID].gateType_ != Gate::PPO) {  // If not PPIs or PPOs.
            ...
            } else if (circuitGates_[gateID].gateType_ == Gate::PPI) {  // If PPIs.
                // Add corresponding fanout.
                circuitGates_[gateID].numFO_ = circuitGates_[j].numFO_;
                circuitGates_[gateID].fanoutVector_.resize(circuitGates_[j].numFO_);
                for (int k = 0; k < circuitGates_[j].numFO_; ++k) {
