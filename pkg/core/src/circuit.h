...
class Circuit {
   public:
    ...
    Circuit(const Circuit& other) {
        ...
        numPO_ = other.numPO_;
        numComb_ = other.numComb_;
        numGate_ = other.numGate_;
        numNet_ = other.numNet_;
        circuitLvl_ = other.circuitLvl_;
        numFrame_ = other.numFrame_;
        timeFrameConnectType_ = other.timeFrameConnectType_;
        totalGate_ = other.totalGate_;
        totalLvl_ = other.totalLvl_;
        circuitGates_ = std::vector<Gate>(other.circuitGates_);
        cellIndexToGateIndex_ = std::vector<int>(other.cellIndexToGateIndex_);
        portIndexToGateIndex_ = std::vector<int>(other.portIndexToGateIndex_);
    }

    // Build the circuit from the netlist.
    bool buildCircuit(IntfNs::Netlist* const pNetlist, const int& numFrame = 1, const TIME_FRAME_CONNECT_TYPE& timeFrameConnectType = CAPTURE);
   ...
   protected:
    ...
    void createCircuitPmt(const int& gateID, const IntfNs::Cell* const cell, const IntfNs::Pmt* const pmt);
    void determineGateType(const int& gateID, const IntfNs::Cell* const cell, const IntfNs::Pmt* const pmt);
