...
ReadPatCmd::ReadPatCmd(const std::string& name, FanMgr* fanMgr)
    : Cmd(name) {
    fanMgr_ = fanMgr;
...
bool AddFaultCmd::addCellFault(const std::string& type, const std::string& cell, const std::string& pin) {
...
bool ReportFaultCmd::exec(const std::vector<std::string>& argv) {
    ...
    for (; it != fanMgr_->fListExtract->faultsInCircuit_.end(); ++it) {
        ...
        if ((*it)->gateID_ == -1) {  // CK
            std::cout << "CK";
        } else if ((*it)->gateID_ == -2) {  // test_si
            std::cout << "test_si";
        } else if ((*it)->gateID_ == -3) {  // test_so
            std::cout << "test_so";
        } else if ((*it)->gateID_ == -4) {  // test_se
            std::cout << "test_se";
...
AddPinConsCmd::AddPinConsCmd(const std::string& name, FanMgr* fanMgr)
    : Cmd(name) {
    fanMgr_ = fanMgr;
    optMgr_.setName(name);
...
bool RunAtpgCmd::exec(const std::vector<std::string>& argv) {
    ...
    if (!fanMgr_->cir) {
        std::cerr << "**ERROR RunFaultSimCmd::exec(): circuit neededhhhh\n";
        return false;
    }
    ...
    fanMgr_->atpg->parallelsetupCircuitParameter();
    fanMgr_->pcoll->init(fanMgr_->cir);
    for (Fault* pFault : fanMgr_->fListExtract->faultsInCircuit_) {
    ...
    std::vector<PatternProcessor*> parallelPatternProcessors;
    std::vector<std::thread> threads;
    std::vector<Pattern> patterns;
    int count = 1;

    for (Fault* fault : originalFaultPtrList) {
        FaultPtrList faultListforParallel;
        faultListforParallel.push_back(fault);

        CoreNs::Circuit* parallelCir = new CoreNs::Circuit();
        ...
        // CoreNs::PatternProcessor* parallelPatternProcessor;
        // parallelPatternProcessor = new CoreNs::PatternProcessor;
        ...
        CoreNs::Pattern pattern(parallelCir);
        patterns.push_back(pattern);

        const double faultPtrListSize = (double)(faultListforParallel.size());
        int numOfAtpgUntestableFaults = 0;
        numOfAtpgUntestableFaults = 0;

        // threads.push_back(std::thread(&CoreNs::Atpg::StuckAtFaultATPG, parallelAtpg, std::ref(faultListforParallel), std::ref(parallelPatternProcessor), std::ref(numOfAtpgUntestableFaults)));
        threads.push_back(std::thread(&CoreNs::Atpg::parallelStuckAtFaultATPG, parallelAtpg, std::ref(faultListforParallel), std::ref(numOfAtpgUntestableFaults), std::ref(pattern)));
    }
    ...
    //     fanMgr_->pcoll->patternVector_.push_back(parallelPatternProcessors[i]->patternVector_[0]);
    // }

    // list;
    // for () {
    //     list = fanMgr_->fListExtract(fault);
    //     pcoll = fanMgr_->pcoll;
    //     list.append(pcoll);
    //     thread(fanMgr_->atpg->generatePatternSet(pcoll, list, true));
    // }
    // waitall();
    // for (list) {
...
WriteStilCmd::WriteStilCmd(const std::string& name, FanMgr* fanMgr)
    : Cmd(name) {
    fanMgr_ = fanMgr;
    optMgr_.setName(name);
