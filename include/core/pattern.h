...
inline void PatternProcessor::StaticCompression() {
    ...
    for (size_t i = 0; i < size - 1; ++i) {
        ...
        for (size_t j = i + 1; j < size; ++j) {
            ...
            for (int k = 0; k < numPI_; ++k) {  // If any bit of the patterns has different values(one is high and one is low), the patterns are not compatible
                if (((patternVector_[i].PI1_[k] == L) && (patternVector_[j].PI1_[k] == H)) || ((patternVector_[i].PI1_[k] == H) && (patternVector_[j].PI1_[k] == L))) {
                    compatible = false;
                    break;
                }
            }
            if (!(patternVector_[i].PI2_.empty()) && (compatible == true)) {  // If the pattern has second primary input, need to check it too
            ...
            if (!(patternVector_[i].PPI_.empty()) && (compatible == true)) {  // Check ppi
            ...
            if (!(patternVector_[i].SI_.empty()) && (compatible == true)) {  // Check si
